...
<div class="card col-12">
    <div class="card-header pb-0">
        <h4 class="my-0">ویرایش صورت جلسه</h4>
    </div>
    <div class="card-body">
        <form wire:submit="update" class="row">
                ...
                <div wire:ignore>
                    <select wire:model="customer_id"
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->customer->id }}" @selected($meeting->customer_id == $user->customer->id)>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
                @error('customer_id')
            ...
            <div class="form-group p-3 col-md-6">
                ...
                <div wire:ignore>
                    <input type="text" class="form-control" value="{{ $meeting->date }}" placeholder="YYYY-MM-DD" id="flatpickr-date"
                           wire:model="date"/>
                </div>
                @error('date')
                <span class="invalid-feedback">{{ $message }}</span>
